...
- Keep `README.md`, `site/index.html`, `CHANGELOG.md`, `package.json`, `bin/docs-forge.js`, and adapter docs aligned when the workflow, install command, or version changes.
...
- Treat `bin/docs-forge.js` as the public `npx` installer. It must stay dependency-free unless there is a strong reason to add package install overhead.
...
- For installer changes, run `node bin/docs-forge.js --help`, a dry-run Codex install, and a temp project-local Claude/Antigravity install.
